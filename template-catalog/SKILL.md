...
description: "Template discovery and parameter documentation for all QNFO prompt templates. Use when user says find template, list templates, what templates are available, template parameters, or when the agent needs to find the right template for a task."
version: "1.1"
---
...
update_plan([
  {"step": "List available templates", "status": "pending"},
  {"step": "Inspect template parameters", "status": "pending"},
  {"step": "Select appropriate template for task", "status": "pending"},
  {"step": "Fill template with provided arguments", "status": "pending"},
  {"step": "Verify filled output matches format", "status": "pending"}
])
