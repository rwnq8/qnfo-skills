...
`skill_view('web-artifacts-builder')` or `read()` with filesystem path.
...
update_plan([
  {"step": "Define component tree and routing", "status": "pending"},
  {"step": "Set up React + Tailwind + shadcn/ui", "status": "pending"},
  {"step": "Implement state management", "status": "pending"},
  {"step": "Build and test all interactive components", "status": "pending"},
  {"step": "Deploy artifact to target platform", "status": "pending"}
])


# Web Artifacts Builder

> **⚠️ Platform Note:** Scripts use `bash` (Unix). On Windows, use Git Bash, WSL, or equivalent.
> If bash is unavailable, manually set up a Vite + React + TypeScript project and use Parcel for bundling.
