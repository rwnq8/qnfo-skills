...
Test suite is self-contained. Run directly via `python scripts/test_suite.py <args>` (embedded in skill directory). See `test-enforcement` skill for full protocol.
...
**Prompt-level guard, not code-level enforcement.** Defense layers: Priority 0, self-diagnostic, post-hoc audit, Kaizen pattern detection.
