// papers-server v3.1 — with R2 migration endpoint
// Adds: POST /admin/migrate-r2 — batch upload body_md from D1 to R2
...
__name(zenodoRecordUrl, "zenodoRecordUrl");
...
function extractYearMonth(dateStr) {
  ...
  try {
    ...
    if (isNaN(d.getTime())) {
      var parts = String(dateStr).substring(0, 7).split('-');
      return { year: parts[0] || '2025', month: parts[1] || '01' };
    }
    return {
...
function generatePaperMd(paper) {
  ...
  var authors = paper.authors || 'Rowan Brad Quni-Gudzinas';
  var abstract = paper.abstract || '';
  var doi = paper.doi || '';
  var published = (paper.published || '').toString().substring(0, 10);
  ...
  md += '---\n\n';
  md += '*This paper is part of the QNFO research corpus.*\n';
  
  return md;
}
__name(generatePaperMd, "generatePaperMd");
...
async function handleMigrateR2(request, env) {
  ...
  // Simple auth token check (set in env vars)
  if (env.ADMIN_TOKEN && token !== env.ADMIN_TOKEN) {
    return new Response(JSON.stringify({ error: 'Unauthorized', hint: 'Provide ?token= in URL or {"token":"..."} in body' }), {
      status: 401,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
    });
  }
  ...
  try {
    ...
    var countResult = await env.DB.prepare(
      "SELECT COUNT(*) as cnt FROM papers WHERE slug IS NOT NULL AND slug != 'None'"
    ).first();
    var total = countResult ? countResult.cnt : 0;

    // Fetch batch
    var stmt = env.DB.prepare(
      "SELECT slug, title, abstract, body_md, published, doi, authors FROM papers WHERE slug IS NOT NULL AND slug != 'None' ORDER BY created_at DESC LIMIT ? OFFSET ?"
    ...
    for (var i = 0; i < papers.length; i++) {
      ...
      try {
        // Determine content: use body_md if available, else generate from metadata
        var markdown = '';
        ...
        // Build R2 path: qnfo/releases/{year}/{month}/{slug}/paper.md
        var ym = extractYearMonth(paper.published);
        var r2Path = 'qnfo/releases/' + ym.year + '/' + ym.month + '/' + slug + '/paper.md';

        // Upload to R2
        await env.PAPERS_BUCKET.put(r2Path, markdown, {
        ...
        errors.push({ slug: slug, error: e.message });
      }
    }

    var hasMore = (offset + batchSize) < total;
    var nextOffset = hasMore ? offset + batchSize : null;

    return new Response(JSON.stringify({
      status: 'ok',
      batch: { offset: offset, size: papers.length, uploaded: uploaded, skipped: skipped, errors: errors.length },
      totalPapers: total,
      hasMore: hasMore,
      nextOffset: nextOffset,
      progress: Math.round((Math.min(offset + batchSize, total) / total) * 100) + '%',
      errorDetails: errors.slice(0, 5),
      successDetails: details.slice(0, 5)
    }), {
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
    });
  } catch (e) {
...
__name(handleMigrateR2, "handleMigrateR2");
...
__name(serveLlmsTxt, "serveLlmsTxt");
...
function renderPaper(paper, markdown, slug) {
  ...
  var bodyHtml = "";
  if (markdown) {
    bodyHtml = markdown.replace(/^### (.+)$/gm, "<h3>$1</h3>").replace(/^## (.+)$/gm, "<h2>$1</h2>").replace(/^# (.+)$/gm, "<h1>$1</h1>").replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\*(.+?)\*/g, "<em>$1</em>").replace(/`([^`]+)`/g, "<code>$1</code>").replace(/\n\n/g, "</p><p>").replace(/^(.+)$/gm, function(m, text) {
...
__name(renderPaper, "renderPaper");
...
// ============ MAIN FETCH HANDLER ============
var papers_server_v3_1_default = {
  async fetch(request, env) {
    var url = new URL(request.url);
    var path = url.pathname;

    // Admin migration endpoint
    if (path === "/admin/migrate-r2") {
      return handleMigrateR2(request, env);
    }
    ...
    // Health check
    if (path === "/health") {
      return new Response(JSON.stringify({ status: "ok", worker: "papers-server", version: "3.1", bindings: { db: !!env.DB, r2: !!env.PAPERS_BUCKET } }), {
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
      });
    }
    ...
    if (paperMatch) {
      ...
      try {
        var paperStmt = env.DB.prepare("SELECT * FROM papers WHERE slug = ? OR id = ?").bind(slug, slug);
        var paperResult = await paperStmt.first();
        if (!paperResult) {
          return new Response(renderNotFound(slug), { status: 404, headers: { "Content-Type": "text/html; charset=utf-8" } });
        }

        // Try R2 for full-text markdown
        var markdown = null;
        ...
        for (var i = 0; i < r2Paths.length; i++) {
          try {
            ...
            if (obj) {
              ...
              console.log("R2 hit:", r2Paths[i], "size:", markdown.length);
              break;
            }
          } catch (e) {
            console.error("R2 fetch failed:", r2Paths[i], e.message);
          }
        }

        return new Response(renderPaper(paperResult, markdown, slug), { headers: { "Content-Type": "text/html; charset=utf-8" } });
      } catch (e) {
    ...
    if (path === "/" || path === "") {
      try {
        ...
        return new Response(renderIndex(papers, { total, page: pageParam, perPage, search }), { headers: { "Content-Type": "text/html; charset=utf-8" } });
      } catch (e) {
...
export {
  papers_server_v3_1_default as default
};
