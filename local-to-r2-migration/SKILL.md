...
`skill_view('local-to-r2-migration')` or `read()` with filesystem path.
...
# Local-to-R2 Migration Skill (v1.1)
...
update_plan([
  ...
  {"step": "Phase 1: Scan and classify local files", "status": "pending"},

  {"step": "Phase 2: Purge orphaned _* files and build artifacts", "status": "pending"},
  ...
  {"step": "Phase 5: Clean up ephemeral scripts (_*.py)", "status": "pending"},
...
| Session start (autonomous) | Auto-detect files from prior sessions ? trigger full Phase 1 scan |
...
- Discovery via D1: `npx wrangler d1 execute qnfo-audit --remote --command "SELECT COUNT(*) FROM discovery_projects" -y` and `npx wrangler d1 execute portfolio-state --remote --command "SELECT COUNT(*) FROM resources" -y` (R2 index DEPRECATED)
...
*Local-to-R2 Migration Skill v1.1 — Scan, classify, purge, upload, index, clean. Thin-client enforcement tool.*
